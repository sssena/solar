...
let GetTokenInfo = async () => {
    ...
    data = fs.readFileSync(data_path, 'utf-8'); // bytecode 추출
    //test
    road_address = await main_contract.getRoadMapPollAddress(0, {
        from: owner
    });
...
let DeployNextRoadmap = async () => {
    ...
    let road_data;
    //test
    road_data = await main_contract.getRoadmapPollParams(1, {
        from: owner
    });
    if (road_data[2] != 0) {
...
let WinTransferCrp = async () => {
    ...
    //test
    let transfer_crp = await main_contract.getRoadmapPollParams(0, {
        from: owner
    });
