...
let params = fs.readFileSync(args_dir + '/deploy_CrpPollRoadmap.json');
...
const start_tm = args.pollstart; // 투표 시작시간 (epoch time)
const end_tm = args.pollend; // 투표 끝시간 (epoch time)
...
let roadmap = web3.eth.contract(JSON.parse(abi));
...
/**
 * CrpPollRoadmap Contract deploy를 수행하는 함수이다. 아래의 절차에 따라 진행된다.
 * 1. unlock Account 수행
 * 2. roadmap.new 수행
...
let RoadmapDeploy = async () => {
    try {
        ...
        let data_with_params = roadmap.new.getData(start_tm, end_tm, main_addr, {data: data}); // parameter까지 고려된 bytecode 추출
        let gas = web3.eth.estimateGas({data: data_with_params}); // gas값 계산
        let contract = await roadmap.new(start_tm, end_tm, main_addr, {from: owner, data: data, gas: gas, mca: main_addr}); // Roadmap Deploy를 위한 tx 생성
        console.log('TX (Deploy Roadmap-poll Contract) Hash=[' + contract.transactionHash + ']');
        do {
...
let RunProc = async () => {
    try {
        ...
        console.log('* param5 (poll start time):.' + args.pollstart);
        console.log('* param6 (poll end time):...' + args.pollend);
        await RoadmapDeploy();
    } catch(err) {}
}
RunProc();