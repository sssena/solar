...
let params = fs.readFileSync(args_dir + '/deploy_CrpFund.json');
...
/* global variable 정의 */
let abi_path = path.resolve(__dirname, 'abi', 'CrpFund.abi'); // abi가 저장된 file path
...
/**
 * CrpFund Contract deploy를 수행하는 함수이다. 아래의 절차에 따라 진행된다.
 * 1. unlock Account 수행
 * 2. Fund.new 수행
...
let FundDeploy = async () => {
    try {
        ...
        let data_with_params = Fund.new.getData(main_addr, {data: data}); // parameter까지 고려된 bytecode 추출
        let gas = web3.eth.estimateGas({data: data_with_params}); // gas값 계산
        let contract = await Fund.new(main_addr, {data: data, from: owner, gas: gas, mca: main_addr}); // Fund Deploy를 위한 tx 생성
        console.log('TX (Deploy Fund Contract) Hash=[' + contract.transactionHash + ']');
