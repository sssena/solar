...
function Usage() {
    console.log("Usage --> ");
    console.log("    node TestTokenEnable.js [argv1] [argv2] [argv3]");
...
let RunProc = async () => {
    try {
        ...
        if(await act.tryActions(api_token.enableToken, showProgress, false, 4, true, owner, passwd, tokenaddr) == false) {
            throw new Error("Failed! (enableToken)");
        }
