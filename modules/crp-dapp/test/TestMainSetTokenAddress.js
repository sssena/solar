...
function Usage() {
    console.log("Usage --> ");
    console.log("    node TestMainSetTokenAddress.js [argv1] [argv2] [argv3] [argv4]");
    console.log("....argv1: Owner 계정 (Index)");
    console.log("....argv2: Owner 암호");
    console.log("....argv3: 메인 컨트랙트 주소");
    console.log("....argv4: 토큰 컨트랙트 주소");
}
...
let RunProc = async () => {
    try {
        ...
        console.log('* Param04 (CrpToken Address):.......' + process.argv[5]);
        ...
        const tokenaddr = process.argv[5]; // 토큰 컨트랙트 주소
        console.log('Owner Account: [' + owner + ']');
        let mainobj = await api_main.getObject(mainaddr);
        if(await act.tryActions(api_main.setTokenAddress, showProgress, false, 5, true, owner, passwd, mainaddr, tokenaddr) == false) {
            throw new Error("Failed! (setTokenAddress)");
        }
        console.log("### 등록된 토큰 주소 ==> [" +mainobj.token_addr() + "]");
    } catch(err) {
        console.log(err);
    }
}
