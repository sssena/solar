...
function Usage() {
    console.log("Usage --> ");
    console.log("    node TestTokenTransfer.js [argv1] [argv2] [argv3] [argv4] [argv5]");
    console.log("....argv1: 토큰을 전송할 계정 (Index)");
    console.log("....argv2: 토큰을 전송할 계정의 암호");
    console.log("....argv3: 토큰 컨트랙트 주소");
    console.log("....argv4: 토큰을 받을 계정");
...
let RunProc = async () => {
    try {
        ...
        console.log('* Param04 (Account to Receive):........' + process.argv[5]);
        ...
        const to = process.argv[5]; // 토큰 받을 계정
        ...
        if(await act.tryActions(api_token.transfer, showProgress, false, 6, true, owner, passwd, tokenaddr, to, utils.getWei(amount)) == false) {
