...
let RunProc = async () => {
    try {
        ...
        let params = fs.readFileSync(args_dir + '/scn10.json');
        ...
        for (i = 1; i<Object.keys(args).length; ++i){
            ...
            if(await act.tryActions(api_withdraw.polling, showProgress, false, 5, true, args[i].holder, args[i].passwd, args[0].ca, args[i].vote) == false) {
