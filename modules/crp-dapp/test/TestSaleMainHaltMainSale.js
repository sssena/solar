...
function Usage() {
    console.log("Usage --> ");
    console.log("    node test/TestSaleMainHaltMainSale.js [argv1] [argv2] [argv3]");
...
let RunProc = async () => {
    try {
        ...
        if (await act.tryActions(api_salemain.haltMainSale, showProgress, 4, true, owner, passwd, salemain) == false) {
            throw new Error("Failed! (haltMainSale)");
        }
    } catch (err) {
        console.log(err);
    }
