...
let RunProc = async () => {
    try {
        ...
        console.log('* Param04 (Account to be locked):...' + process.argv[5]);
        ...
        const lockaddr = process.argv[5]; // 잠글 계정
        console.log('Owner Account: [' + owner + ']');
        let tokenobj = await api_token.getObject(tokenaddr);
        console.log("### 계정 잠금 상태 [" + lockaddr + "] ==> ", tokenobj.locks(lockaddr));
        if(await act.tryActions(api_token.lockAddr, showProgress, false, 5, true, owner, passwd, tokenaddr, lockaddr) == false) {
            throw new Error("Failed! (lockAddr)");
        }
        console.log("### 계정 잠금 상태 [" + lockaddr + "] ==> ", tokenobj.locks(lockaddr), "done!");
    } catch(err) {
        console.log(err);
    }
}
