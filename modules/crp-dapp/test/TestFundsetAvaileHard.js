...
function Usage() {
    console.log("Usage --> ");
    console.log("    node test/TestFundsetAvaileHard.js [argv1] [argv2] [argv3] [argv4]");
    console.log("....argv1: Owner 계정 (Index)");
    console.log("....argv2: Owner 암호");
    console.log("....argv3: Crpfund 컨트랙트 주소");
    console.log("....argv4: 설정 할 CRP 양");    
}
...
let RunProc = async () => {
    try {
        ...
        console.log('* Param04 (set hardfund amount):............' + process.argv[5]);
        ...
        if (await act.tryActions(api_fund.setAvaileHard, showProgress, 5, true, owner, passwd, fund, amount) == false) {
            throw new Error("Failed! (setAvaileHard)");
        }
    } catch (err) {
        console.log(err);
    }
