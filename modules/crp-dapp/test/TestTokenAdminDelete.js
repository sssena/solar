...
function Usage() {
    console.log("Usage --> ");
    console.log("    node TestTokenAdminDelete.js [argv1] [argv2] [argv3] [argv4]");
    console.log("....argv1: Owner 계정 (Index)");
    console.log("....argv2: Owner 암호");
    console.log("....argv3: 토큰 컨트랙트 주소");
    console.log("....argv4: 관리자 권한 박탈할 계정");
}
...
let RunProc = async () => {
    try {
        ...
        console.log('* Param04 (Account to delete admin):...' + process.argv[5]);
        ...
        const deladmin = process.argv[5]; // 관리자 권한 박탈할 계정
        console.log('Owner Account: [' + owner + ']');
        let tokenobj = await api_token.getObject(tokenaddr);
        console.log("### 관리자 권한 상태 [" + deladmin + "] ==> ", tokenobj.admins(deladmin));
        if(await act.tryActions(api_token.deleteAdmin, showProgress, false, 5, true, owner, passwd, tokenaddr, deladmin) == false) {
            throw new Error("Failed! (deleteAdmin)");
        }
        console.log("### 관리자 권한 상태 [" + deladmin + "] ==> ", tokenobj.admins(deladmin), "done!");
    } catch(err) {
        console.log(err);
    }
}
