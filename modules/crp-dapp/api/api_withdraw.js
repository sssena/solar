...
/**
 * CrpPollwithdraw 컨트랙트 오브젝트를 반환하는 함수이다.
 * 
 * @param _ca 컨트랙트 주소
 * @author sykang
 */
let getObject = async function(_ca) {
    return contract.withdraw.at(_ca);
}

/**
 * CrpPollwithdraw contract에서 투표하는 함수이다. 아래의 절차에 따라 진행된다.
 *
 * 1. unlock Account 진행
 * 2. gas값 계산 ( withdraw/ polling )
 * 3. tx 생성( withdraw / polling )
...
let polling = async function(_wait, _voter, _passwd, _ca, _vote){
    try{
        let count = 0;
        let withdraw = contract.withdraw.at(_ca);
        console.log("CrpPollwithdraw 컨트랙트 polling 수행 중...");
        await provider.web3.personal.unlockAccount(_voter, _passwd); // 1. unlock Account 수행
        let gas = await withdraw.polling.estimateGas(_vote, {from: _voter}); // 2. gas값 계산
        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await withdraw.polling(_vote, {from: _voter,  gas: gas, txType: "Normal"}); // 3. tx 생성
 ...
 /**
 * CrpPollwithdraw contract에서 투표를 취소 하는 함수이다. 아래의 절차에 따라 진행된다.
 * 
 * 1. unlock Account 진행
 * 2. gas값 계산 ( withdraw / cancelPoll )
 * 3. tx 생성( withdraw / cancelPoll )
 * 4. getTransactionReceipt를 통해, 발행된 tx가 블록에 실렸는지 확인
 * 5. lock Account 수행
 * 
 * @param _wait 블럭에 실릴 때까지 기다릴지 여부
...
let cancelPoll = async function(_wait, _voter, _passwd, _ca){
    try{
        let count = 0;
        let withdraw = contract.withdraw.at(_ca);
        console.log("CrpPollwithdraw 컨트랙트 cancelPoll 수행 중...");
        await provider.web3.personal.unlockAccount(_voter, _passwd); // 1. unlock Account 수행
        let gas = await withdraw.cancelPoll.estimateGas({from: _voter}); // 2. gas 값 계산

        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await withdraw.cancelPoll({from: _voter,  gas: gas, txType: "Normal"});
        console.log('TX Hash=[' + tx + ']');
        if(_wait) {
            let receipt;
            do {
...
/**
 * CrpPollwithdraw contract에서 투표 무게를 계산하는 함수이다. 아래의 절차에 따라 진행된다.
 * 
 * 1. unlock Account 진행
 * 2. gas값 계산 ( withdraw / setAmount )
 * 3. tx 생성( withdraw / setAmount )
 * 4. getTransactionReceipt를 통해, 발행된 tx가 블록에 실렸는지 확인
 * 5. lock Account 수행
 * 
 * @param _wait 블럭에 실릴 때까지 기다릴지 여부
...
let setAmount = async function(_wait, _owner, _passwd, _ca, _voter, _amount ){
    try{
        let count = 0;
        let withdraw = contract.withdraw.at(_ca);
        console.log("CrpPollwithdraw 컨트랙트 setAmount 수행 중...");
        await provider.web3.personal.unlockAccount(_owner, _passwd); // 1. unlock Account 수행
        let gas = await withdraw.setAmount.estimateGas(_voter, _amount, {from: _owner});
        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await withdraw.setAmount(_voter, _amount, {from: _owner,  gas: gas, txType: "Normal"});
        console.log('TX Hash=[' + tx + ']');
        if(_wait) {
            let receipt;
            do {
...
/**
 * CrpPollwithdraw contract에서 투표를 정산하는 함수이다. 아래의 절차에 따라 진행된다.
 * 
 * 1. unlock Account 진행
 * 2. gas값 계산 ( withdraw / settlePoll )
 * 3. tx 생성( withdraw / settlePoll )
 * 4. getTransactionReceipt를 통해, 발행된 tx가 블록에 실렸는지 확인
 * 5. lock Account 수행
 * 
 * @param _wait 블럭에 실릴 때까지 기다릴지 여부
 ...
 * @param _passwd owner 암호
 * @param _ca 컨트렉트 주소
 * @param _addr_count 총 토큰 보유 계좌 수
 * @param _total_token 토큰 총 공급량
 * @param _total_agree 총 찬성 토큰 수
 * @author sykang
 */
let settlePoll = async function(_wait, _owner, _passwd, _ca, _addr_count, _total_token, _total_agree){
    try{
        let count = 0;
        let withdraw = contract.withdraw.at(_ca);
        console.log("CrpPollwithdraw 컨트랙트 settle 수행 중...");
        await provider.web3.personal.unlockAccount(_owner, _passwd); // 1. unlock Account 수행
        let gas = await withdraw.settlePoll.estimateGas(_addr_count, _total_token, _total_agree, {from: _owner});
        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await withdraw.settlePoll(_addr_count, _total_token, _total_agree, {from: _owner,  gas: gas, txType: "Normal"});
        console.log('TX Hash=[' + tx + ']');
        if(_wait) {
            let receipt;
            do {
