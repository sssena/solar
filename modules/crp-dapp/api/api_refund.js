...
/**
 * CrpPollRefund 컨트랙트 오브젝트를 반환하는 함수이다.
 * 
 * @param _ca 컨트랙트 주소
 * @author jhhong
 */
let getObject = async function(_ca) {
    return contract.refund.at(_ca);
}

/**
 * CrpPollRefund contract에서 투표하는 함수이다. 아래의 절차에 따라 진행된다.
 * 
 * 1. unlock Account 진행
 * 2. gas값 계산 (refund/ polling )
 * 3. tx 생성(refund / polling)
...
let polling = async function(_wait, _voter, _passwd, _ca, _vote){
    try{
        let count = 0;
        let refund = contract.refund.at(_ca);
        console.log("CrpPollRefund 컨트랙트 polling 수행 중...");
        await provider.web3.personal.unlockAccount(_voter, _passwd); // 1. unlock Account 수행
        let gas = await refund.polling.estimateGas(_vote, {from: _voter});
        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await refund.polling(_vote, {from: _voter,  gas: gas, txType: "Normal"});
 ...
 /**
 * CrpPollRefund contract에서 투표를 취소 하는 함수이다. 아래의 절차에 따라 진행된다.
 * 
 * 1. unlock Account 진행
 * 2. gas값 계산 ( refund / cancelPoll )
 * 3. tx 생성( refund / caocelPoll )
 * 4. getTransactionReceipt를 통해, 발행된 tx가 블록에 실렸는지 확인
 * 5. lock Account 수행
 * 
 * @param _wait 블럭에 실릴 때까지 기다릴지 여부
...
let cancelPoll = async function(_wait, _voter, _passwd, _ca){
    try{
        let count = 0;
        let refund = contract.refund.at(_ca);
        console.log("CrpPollRefund 컨트랙트 cancelPoll 수행 중...");
        await provider.web3.personal.unlockAccount(_voter, _passwd); // 1. unlock Account 수행
        let gas = await refund.cancelPoll.estimateGas({from: _voter});
        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await refund.cancelPoll({from: _voter,  gas: gas, txType: "Normal"});
        console.log('TX Hash=[' + tx + ']');
        if(_wait) {
            let receipt;
            do {
...
/**
 * CrpPollRefund contract에서 투표 무게를 계산하는 함수이다. 아래의 절차에 따라 진행된다.
 * 
 * 1. unlock Account 진행
 * 2. gas값 계산 ( refund / setAmount )
 * 3. tx 생성( refund / setAmount )
 * 4. getTransactionReceipt를 통해, 발행된 tx가 블록에 실렸는지 확인
 * 5. lock Account 수행
 * 
 * @param _wait 블럭에 실릴 때까지 기다릴지 여부
...
let setAmount = async function(_wait, _owner, _passwd, _ca, _voter, _amount ){
    try{
        let count = 0;
        let refund = contract.refund.at(_ca);
        console.log("CrpPollRefund 컨트랙트 setAmount 수행 중...");
        await provider.web3.personal.unlockAccount(_owner, _passwd); // 1. unlock Account 수행
        let gas = await refund.setAmount.estimateGas(_voter, _amount, {from: _owner});
        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await refund.setAmount(_voter, _amount, {from: _owner,  gas: gas, txType: "Normal"});
        console.log('TX Hash=[' + tx + ']');
        if(_wait) {
            let receipt;
            do {
...
/**
 * CrpPollRefund contract에서 투표를 정산하는 함수이다. 아래의 절차에 따라 진행된다.
 * 
 * 1. unlock Account 진행
 * 2. gas값 계산 ( refund / settlePoll )
 * 3. tx 생성( refund / settlePoll )
 * 4. getTransactionReceipt를 통해, 발행된 tx가 블록에 실렸는지 확인
 * 5. lock Account 수행
 * 
 * @param _wait 블럭에 실릴 때까지 기다릴지 여부
...
let settlePoll = async function(_wait, _owner, _passwd, _ca, _addr_count, _total_token, _total_agree){
    try{
        let count = 0;
        let refund = contract.refund.at(_ca);
        console.log("CrpPollRefund 컨트랙트 settle 수행 중...");
        await provider.web3.personal.unlockAccount(_owner, _passwd); // 1. unlock Account 수행
        let gas = await refund.settlePoll.estimateGas(_addr_count, _total_token, _total_agree, {from: _owner});
        console.log("측정된 가스값 ==> [" + gas + "]");
        let tx = await refund.settlePoll(_addr_count, _total_token, _total_agree, {from: _owner,  gas: gas, txType: "Normal"});
        console.log('TX Hash=[' + tx + ']');
        if(_wait) {
            let receipt;
            do {
