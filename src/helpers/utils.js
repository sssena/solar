 ...
 * @return hh:mm:ss with duration
 * */
function humanizeSecondsDuration( duration ){
    ...
    // make string
    let formatted = "";
    formatted += ( zeropad( h, 10 ) + ":");
    formatted += ( zeropad( m, 10 ) + ":");
    formatted += ( zeropad( s, 10 ));
...
function zeropad( number, base ){
    let length = ( String(base).length - String(number).length ) + 1;
    return length > 0 ? new Array( length ).join('0') + number : number ;
}

/**
 * crp값을 wei값(10^18)으로 변환한다.
 *
 * @param _val wei값으로 변환할 값 (crp)
 * @return wei값
 * @author jhhong
 */
function getWei(_val) {
 ...
 * wei값을 crp값으로 변환한다.
 *
 * @param _val crp값으로 변환할 값 (wei)
 * @return ehter값
 * @author jhhong
 *         sena : change 'ether' -> 'crp'
 */
function getCrp(_val) {
    let value;
    let count;
    do {
        if(_val == '0') {
